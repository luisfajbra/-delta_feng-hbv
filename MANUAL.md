# Manual Completo — dPL-HBV (Feng et al. 2022)

> **Guía definitiva** para entender cada archivo, cada función, cada línea relevante del proyecto.
> Este manual te prepara para defender y explicar el código en cualquier contexto.

---

## Tabla de Contenidos

1. [Conceptos Fundamentales del Paper](#1-conceptos-fundamentales-del-paper)
2. [El Modelo HBV — Física del Proceso](#2-el-modelo-hbv--física-del-proceso)
3. [Archivo 1: `delta_HBV_Feng2022.py` — Comparación HBV vs δ-HBV](#3-archivo-1-delta_hbv_feng2022py)
4. [Archivo 2: `dpl_hbv_demo.py` — Red Neural + HBV Diferenciable](#4-archivo-2-dpl_hbv_demopy)
5. [Archivo 3: `dpl_hbv_paper.py` — Replicación Fiel del Paper](#5-archivo-3-dpl_hbv_paperpy)
6. [Glosario de Parámetros](#6-glosario-de-parámetros)
7. [Diagrama de Flujo Completo](#7-diagrama-de-flujo-completo)

---

## 1. Conceptos Fundamentales del Paper

### 1.1 ¿Qué problema resuelve el paper?

En hidrología existen dos familias de modelos:

| Familia | Ejemplo | Precisión (NSE) | Interpretabilidad |
|---------|---------|-----------------|-------------------|
| **Físicos/conceptuales** | HBV, mHM, SAC-SMA | ~0.53 | Alta (sabés qué pasa internamente) |
| **Deep learning** | LSTM | ~0.748 | Nula (caja negra) |

**El paper propone un híbrido**: usar el HBV como esqueleto físico pero hacerlo **diferenciable** (implementarlo en PyTorch para que los gradientes fluyan), y usar una LSTM para **aprender los parámetros** desde los datos.

### 1.2 ¿Qué significa "diferenciable"?

En PyTorch, cada operación matemática registra cómo contribuyó al resultado. Esto permite calcular **gradientes** — qué tanto cada parámetro influyó en el error. Con esos gradientes, podés ajustar los parámetros automáticamente con backpropagation.

**Implicación**: El HBV deja de ser un modelo que "calibrás a mano" y se convierte en un modelo que "se entrena solo" igual que una red neural.

### 1.3 La arquitectura del paper (Figura 1 del paper)

```
Forzamientos (P, T, PET) + Atributos de cuenca
              │
              ▼
     ┌─────────────────┐
     │  LSTM (g_A)     │  ← "Inversa": predice parámetros, no caudal
     │                 │
     │  Input: [T, 30] │  ← 3 forcing + 27 atributos CAMELS
     │  Output: [15]   │  ← 13 HBV params + 2 routing params
     └────────┬────────┘
              │ sigmoid → escala a rango físico
              ▼
     ┌─────────────────┐
     │  HBV Forward    │  ← Modelo físico diferenciable
     │  (5 estados)    │
     │  (13 params)    │
     └────────┬────────┘
              ▼
        ...
        Loss (RMSE)
              │
              ▼ backprop
        Actualizar pesos LSTM
```

### 1.4 Parametrización dinámica (δ models)

El paper introduce la notación **δ** para modelos con parámetros que **varían en el tiempo**:

| Notación | Descripción | Parámetros |
|----------|-------------|------------|
| **δ₁** | Todos estáticos (HBV clásico) | 12-13 fijos |
| **δₙ(βₜ)** | β varía cada día | β(t) + 11-12 fijos |
| **δₙ(γₜ, βₜ)** | β Y γ varían cada día | β(t), γ(t) + 11 fijos |

- **β(t)**: refleja la memoria hídrica de la cuenca (qué tan "llena" está). Máximo en septiembre, mínimo en marzo/abril.
- **γ(t)**: refleja la fenología vegetal (qué tan activa está la vegetación). Máximo en verano.

---

## 2. El Modelo HBV — Física del Proceso

### 2.1 Los 5 estados (almacenamientos)

```
        ┌─────────┐
        │   Sp    │  ← Snowpack (nieve sólida acumulada)
        └────┬────┘
             │
        ┌────┴────┐
        │  Sliq   │  ← Agua líquida en la nieve
        └────┬────┘
             │ Isnow (drenaje al suelo)
             ▼
    ┌────────────────┐
    │       Ss       │  ← Humedad del suelo (soil moisture)
    └──┬─────────┬───┘
       │         │
  Peff │      Ex │ (exceso por encima de FC)
       ▼         ▼
    ┌────────────────┐
    │      Suz       │  ← Zona superior (upper zone)
    └──┬─────────┬───┘
       │         │
  Perc │         │ Q0, Q1
       ▼         ▼
    ┌────────────────┐     ┌──────────┐
    │      Slz       │────→│ Q2       │  ← Flujo base (baseflow)
    └────────────────┘     └──────────┘
     (aquifer/zona inferior)
```

### 2.2 Los 13 parámetros y su significado físico

| # | Parámetro | Rango | Unidad | Qué controla |
|---|-----------|-------|--------|--------------|
| 1 | **TT** | [-2.5, 2.5] | °C | Temp umbral: ¿llueve o nieva? |
| 2 | **CFMAX** | [0.5, 10] | mm/°C/d | Cuánta nieve se derrite por cada grado |
| 3 | **CFR** | [0, 0.1] | - | Cuánto se recongela el agua líquida |
| 4 | **CWH** | [0, 0.2] | - | Cuánta agua líquida retiene el snowpack |
| 5 | **FC** | [50, 1000] | mm | Capacidad máxima del suelo |
| 6 | **LP** | [0.2, 1.0] | - | Umbral para reducir ET |
| 7 | **BETA** | [1, 6] | - | Forma de la curva de respuesta del suelo |
| 8 | **PERC** | [0, 10] | mm/d | Máxima percolación al acuífero |
| 9 | **UZL** | [0, 100] | mm | Umbral para flujo rápido |
| 10 | **K0** | [0.05, 0.9] | 1/d | Velocidad del flujo rápido |
| 11 | **K1** | [0.01, 0.5] | 1/d | Velocidad del flujo intermedio |
| 12 | **K2** | [0.001, 0.2] | 1/d | Velocidad del flujo base |
| 13 | **BETAET** | [0.3, 5] | - | Forma de la ecuación de ET |

### 2.3 Las ecuaciones paso a paso

**Cada día**, el HBV ejecuta estos módulos en orden:

#### Módulo 1: Nieve (Snow)
```
Si T[t] ≤ TT:  Ps = P[t]  (nieve), Pr = 0
Si T[t] > TT:  Ps = 0,     Pr = P[t]  (lluvia)

smelt = max(0, CFMAX × (T[t] - TT))     # cuánto se derrite
Rfz   = max(0, CFR × (TT - T[t]) × Sliq) # cuánto se recongela

Sp   = max(0, Sp + Ps + Rfz - smelt)     # update snowpack
Sliq = max(0, Sliq + smelt - Rfz)        # update agua líquida
Isnow = max(0, Sliq - CWH × Sp)          # drenaje al suelo
Sliq -= Isnow
```

#### Módulo 2: Suelo (Soil)
```
W     = min((Ss / FC)^BETA, 1)    # factor de respuesta del suelo
Peff  = W × (Pr + Isnow)           # precipitación efectiva → runoff

Ex    = max(0, Ss - FC)            # exceso sobre capacidad de campo

eta   = min((Ss / (FC × LP))^BETAET, 1)  # coeficiente de ET real
ET    = eta × Ep[t]                 # evapotranspiración real

Ss    = clamp(Ss + Pr + Isnow - Peff - Ex - ET, 0, FC)
```

#### Módulo 3: Aguas subterráneas (Groundwater)
```
Perc  = min(PERC, Suz)             # percolación al acuífero
Q0    = max(0, K0 × (Suz - UZL))  # flujo rápido (solo si Suz > UZL)
Q1    = K1 × Suz                   # flujo intermedio

Suz   = max(0, Suz + Peff + Ex - Perc - Q0 - Q1)

Q2    = K2 × Slz                   # flujo base
Slz   = max(0, Slz + Perc - Q2)    # update acuífero

Q_raw = Q0 + Q1 + Q2               # caudal total del día
```

#### Módulo 4: Routing (Gamma Unit Hydrograph)
```
# Convoluciona Q_raw con un hidrograma unitario gamma
# Esto simula el retardo que tiene el agua al llegar al cauce
Q_routed = Q_raw * UH_gamma(a, b)
```
